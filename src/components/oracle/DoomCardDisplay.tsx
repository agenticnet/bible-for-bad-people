...
const OMEN_STYLES = {
  ...
  bleak: "border-muted/40 text-muted",
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      >
        ...
        >
          {/* Card back */}
          <div className="absolute inset-0 flex flex-col items-center justify-center rounded-xl border-2 border-neon-purple/30 bg-gradient-to-br from-shadow via-abyss to-shadow [backface-visibility:hidden]">
            <div className="absolute inset-2 rounded-lg border border-neon-purple/20" />
            <span className="text-3xl opacity-60">🔮</span>
            <p
              className="mt-3 text-[10px] uppercase tracking-[0.3em] text-neon-purple/60"
          ...
          {/* Card front */}
          <div className="absolute inset-0 flex flex-col rounded-xl border-2 border-neon-purple/40 bg-shadow p-4 [backface-visibility:hidden] [transform:rotateY(180deg)]">
            <div className="flex flex-1 flex-col items-center justify-center text-center">
              <span className="mb-3 text-4xl">{card.symbol}</span>
              <h3
                className="mb-2 text-sm font-bold leading-tight text-bone"
                style={{ fontFamily: "var(--font-display)" }}
              >
                {card.name}
              </h3>
              <span
                className={cn(
                  ...
                  OMEN_STYLES[card.omen]
                )}
              >
                {OMEN_LABELS[card.omen]}
              </span>
            </div>
            <p className="mt-2 border-t border-ash pt-2 text-center text-[11px] italic text-muted">
              {card.tagline}
            </p>
          </div>
        </div>
      ...
      {revealed && (
        <p className="max-w-[200px] text-center text-xs leading-relaxed text-bone/80">
          {card.reading}
        </p>
      )}
    </div>
