...
import { Badge, Button, Callout, TearSlider } from "@/components/ui";
import { THUMB_CTA_MIN_HEIGHT, Z_BOTTOM_SHEET as Z_SHEET } from "@/lib/ux/constraints";
...
export default function MysteryReveal({
  ...
  return (
    <AnimatePresence>
      {active && step !== "done" && (
        <motion.div
          className={cn("fixed inset-0 flex flex-col", Z_SHEET)}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        >
          <div className="flex flex-1 flex-col items-center justify-center bg-binding/90 p-6 pb-32">
            ...
            {step === "suspense" && (
              ...
              >
                ...
                <div className="h-32 w-48 animate-pulse rounded-xl border border-wine/30 bg-wine/10" />
            ...
            {step === "reveal" && reward && (
              ...
              >
                ...
                <motion.div
                  className={cn(
                    "flex flex-col items-center rounded-2xl border-2 bg-parchment p-8",
                ...
                >
                  ...
                  <h3 className="text-xl font-bold text-ink">{reward.productName}</h3>
                  {purchasePending && (
                    <p className="mt-2 text-xs text-ink-soft">Securing your vault entry…</p>
                  )}
                </motion.div>
              </motion.div>
            )}
          ...
          {step === "anticipation" && (
            <div className="fixed inset-x-0 bottom-0 flex justify-center border-t border-rule bg-binding/95 px-4 py-6 pb-[max(1.5rem,env(safe-area-inset-bottom))]">
              <TearSlider onComplete={handleTearComplete} />
            </div>
          )}

          {step === "reveal" && reward && (
            <div className="fixed inset-x-0 bottom-0 border-t border-rule bg-parchment/95 px-4 py-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
